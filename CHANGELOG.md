...
- added read-only `client.firewall.ip_filter()` and `client.firewall.port_filter()` for the upstream live-verified lower-level filter list reads; both send the physically confirmed empty-list POST bodies, preserve raw response/list items without inventing a non-empty rule schema, and expose no filter write in this block
...
- `tests/integration/test_reversible_writes.py`, hard-gated by `NR2301_WRITE_INTEGRATION=1`, covering data-roaming, available network-mode, WPS, Wi-Fi Guest/combined-vs-separate, auto-sleep timeout, timed-reboot schedule and UI-language transitions with `try/finally` restoration and final read-back
...
- firewall IP/port filter list reads physically validated through the public SDK on ACIY.3 on 2026-09-08: targeted `test_firewall_reads` exercised `client.firewall.ip_filter()` and `client.firewall.port_filter()` with the exact confirmed empty-list POST bodies and passed `1/1` in 1.82 s; no rule contents, IP addresses or ports were printed and no firewall state was modified
...
- first full read-only physical SDK smoke completed successfully on 2026-08-31 against the USB-connected NR2301 using Python 3.13.5 and `http://zyxel.home`: all 8 integration groups passed in 4.01 s (version, device health, SIM, mobile, LAN/DNS reads, Wi-Fi status, SMS summary and statistics)
...
- corrected timed-reboot time handling after physical ACIY.3 evidence showed `router_get_timed_reboot.time` may be unpadded (`0:0`); the SDK now preserves raw getter text, validates one- or two-digit hour/minute components numerically, canonicalizes writes to `HH:MM`, and compares schedule state semantically across padding differences
