...
class LANNamespace:
    ...
    def set_static_reservations(
        ...
        *,
        ...
        """Replace the complete static DHCP reservation table."""
        ...
        *,
        ...
        for attempt in range(recovery_attempts):
            try:
                actual = self.address(timeout=recovery_timeout)
                last_actual = actual
                actual_router = actual.get("router")
                if (
                    isinstance(actual_router, Mapping)
                    and actual_router.get("lan_ip") == lan_ip
                    and actual_router.get("lan_netmask") == lan_netmask
                ):
                    return actual
            except NR2301Error as exc:
                last_error = exc
