...
def _move_candidates(indexes: list[int], target_group: int) -> list[tuple[str, Any]]:
    a, b = indexes
    return [
        ("COMMA_STRING", f"{a},{b}"),
...
def main() -> None:
    ...
    with NR2301Client(
        ...
        try:
            ...
            for label, contacts_value in _move_candidates(move_indexes, group_b):
            ...
            # The move contacts remain synthetic and are no longer needed.
            for index in move_indexes:
            ...
            for serial, (label, _) in enumerate(_delete_candidates([1, 2]), start=1):
                ...
                # Rebuild the candidate using the real indexes created for this attempt.
